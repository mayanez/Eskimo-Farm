...
void init_invaders() {
    ...
    invaders.enemy[0].speed = 1;
    invaders.enemy[0].sprite_info.x = MAX_X - PIG_DIM;
    invaders.enemy[0].sprite_info.y = MAX_Y - 100;
...
void add_enemy() {
    int index;
	if ((ticks%140 == 0 ) && current_enemy_count < MAX_ENEMIES) {
        ...
        }else if(score <= HEN_SCORE){
            invaders.enemy[index].sprite_info.x = MAX_X - HEN_DIM;
            invaders.enemy[index].sprite_info.y = MAX_Y / 2;
            invaders.enemy[index].sprite_info.id = HEN_ID;
            invaders.enemy[index].sprite_info.dim = HEN_DIM;
